...
async function loadDataWithRetry(retries = CONFIG.MAX_RETRIES) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            lastError = error;
            console.warn(`[Load] Attempt ${attempt}/${retries} failed:`, error.message);
...
function showErrorFallback(error) {
    ...
    placeholders.forEach(el => {
        if (el.textContent.includes('Carregando') || el.textContent.includes('Nenhum')) {
            el.textContent = '⚠️ Não foi possível carregar os dados. Tente novamente.';
...
function setupScrollReveal() {
    const elements = document.querySelectorAll('.card, .panel, .collab-card, .commit-row, .stat-box, .heat-box');
...
function initAnimations() {
    const metrics = [
        { el: document.getElementById('total-commits'), target: parseInt(document.getElementById('total-commits')?.textContent) || 0 },
        { el: document.getElementById('closed-issues'), target: parseInt(document.getElementById('closed-issues')?.textContent) || 0 },
        { el: document.getElementById('total-collabs'), target: parseInt(document.getElementById('total-collabs')?.textContent) || 0 },
    ];
...
function initParallax() {
    ...
    hero.addEventListener('mousemove', (e) => {
        ...
        title.style.transition = 'transform 0.15s cubic-bezier(0.34, 1.56, 0.64, 1)';

        if (subtitle) {
            subtitle.style.transform = `translate(${x * 12}px, ${y * 8}px)`;
            subtitle.style.transition = 'transform 0.2s cubic-bezier(0.34, 1.56, 0.64, 1)';
        }

        if (description) {
            description.style.transform = `translate(${x * 8}px, ${y * 6}px)`;
            description.style.transition = 'transform 0.25s cubic-bezier(0.34, 1.56, 0.64, 1)';
        }
        ...
        if (glow) {
            ...
            glow.style.transition = 'transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)';
        }
    });
...
function initStagger() {
    const elements = document.querySelectorAll('.hero h1, .hero .subtitle, .hero .description, .hero .btn-primary');
...
function initEventListeners() {
    ...
    document.addEventListener('keydown', (event) => {
        ...
        if (event.key === 'Escape' && dom.views.dashboard.classList.contains('is-active')) {
            showLanding();
        }
    });
}
